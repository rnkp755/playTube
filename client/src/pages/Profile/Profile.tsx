    ...
    <Link to={`/playlist/${playlist._id}`} className="card card-hover">
      <div className="relative">
        {playlist.videos.length > 0 ? (
          <img 
            src={typeof playlist.videos[0] === 'object' ? playlist.videos[0].thumbnail : 'https://images.pexels.com/photos/1092671/pexels-photo-1092671.jpeg'} 
            alt={playlist.name} 
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-3">
          <div>
            ...
            <p className="text-white/80 text-sm">{playlist.videos.length} videos</p>
          </div>
        </div>
      </div>
    </Link>
...
const Profile: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'videos' | 'playlists' | 'community'>('videos');
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (isOwner) {
          ...
          setVideos(userVideos.filter(v => v.owner.username === username));
        } else {
          // For non-owner, show only published videos
          const userVideos = await videoService.getVideos();
          setVideos(userVideos.filter(v => v.owner.username === username && v.isPublished));
        }
        
        // Fetch playlists (simulated)
        if (currentUser) {
          const userPlaylists = await playlistService.getUserPlaylists(currentUser._id);
          setPlaylists(userPlaylists);
        }
        
        // Fetch tweets (simulated)
        if (currentUser) {
          const userTweets = await tweetService.getUserTweets(currentUser._id);
  ...
  if (error || !user) {
    return (
      <div className="text-center py-10">
        ...
        <p className="mb-4">{error || `The user ${username} doesn't exist.`}</p>
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <div className="mt-2 flex flex-wrap justify-center sm:justify-start gap-3">
              ...
              <div>
                ...
                <span className="text-muted ml-1">playlists</span>
              </div>
              <div>
                <span className="font-semibold">1.2K</span> 
                <span className="text-muted ml-1">subscribers</span>
              </div>
            </div>
          </div>
      ...
      <div className="mt-6 px-4 sm:px-6">
        ...
        {activeTab === 'community' && (
          <div>
            {tweets.length === 0 ? (
              <div className="text-center py-10 bg-card rounded-lg">
                <p className="text-lg mb-4">No community posts yet.</p>
                {isOwner && (
